...
public interface IThirdPartyApiService
{
    ...
    Task<string> HandleApiData<TRequest>(TRequest request) where TRequest : BaseThirdPartyApiRequest;
    ...
    /// </exception>
    Task<TResponse> HandleApiData<TRequest, TResponse>(TRequest request) where TRequest : BaseThirdPartyApiRequest;
