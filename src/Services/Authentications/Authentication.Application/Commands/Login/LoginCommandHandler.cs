...
public class LoginCommandHandler : ICommandHandler<LoginCommand, ResponseDto<LoginResponseDto>>
{
    ...
    public async ValueTask<ResponseDto<LoginResponseDto>> Handle(LoginCommand request, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation(LOCAL_LOGIN_FLOW_STEP1_LOCK_CHECKED);
        ...
        try
        {
            ...
            _logger.LogInformation(LOCAL_LOGIN_FLOW_STEP2_CREDENTIALS_ACCEPTED);
        }
        catch (ApiException ex) when (ex.ErrorCode == AUTH_INVALID_CREDENTIALS)
        {
            ...
            _logger.LogWarning(ex, LOCAL_LOGIN_FLOW_INVALID_ATTEMPT_COUNTED);
        ...
        _logger.LogInformation(LOCAL_LOGIN_FLOW_STEP3_ATTEMPTS_RESET);
        _logger.LogInformation(LOCAL_LOGIN_COMPLETED);
